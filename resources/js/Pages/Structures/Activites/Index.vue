...
<template>
    ...
    <AppLayout>
            ...
            <div class="flex flex-col items-start justify-between md:flex-row md:items-center">
                ...
                <div class="w-full mt-4 md:mt-0 md:w-1/4">
                    <div class="flex flex-col justify-between space-y-4 md:ml-4 md:space-y-6">
                        ...
                        <Link :href="route('structures.edit', structure.slug)" v-if="can.update"
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div>
                    <form @submit.prevent="submit" autocomplete="off">
                        <div class="min-h-screen shadow-md shadow-sky-700 sm:overflow-hidden sm:rounded-md">
                            <div
                                class="flex flex-col items-start justify-start w-full px-4 py-5 bg-white md:flex-row md:px-6 md:py-10">
                                <!-- discipline -->
                                <AutocompleteActiviteFormSmall class="w-full h-full md:w-1/2" :disciplines="listDisciplines"
                                    :dejaUsedDisciplines="dejaUsedDisciplinesRef" :errors="form.errors"
                                    v-model:discipline="form.discipline_id" :selectedDiscipline="selectedDiscipline"
                                    @update:modelValue="(discipline) => (form.discipline_id = discipline)" />
                                <!-- disciplines similaires -->
                                <section v-if="activiteSimilairesList.length > 0" class="mx-auto md:w-1/2">
                                    <h2 class="mb-4 text-lg font-medium text-gray-700">
                                        Les disciplines similaires
                                    </h2>
                                    <div class="grid grid-flow-col gap-4 text-gray-700 auto-cols-auto">
                                        <div v-for="discipline in activiteSimilairesList" :key="discipline.id"
                                            :index="discipline.id" :class="{
                                            ...
                                            <button type="button" @click="
                                                ...
                                                ">
                                                {{ discipline.name }}
                                                <span v-if="dejaUsedDisciplinesRef.includes(
                                                    discipline.id
                                                )
                                                    " class="text-xs italic text-gray-400">(déjà sélectionné)</span>
                                            </button>
                                        </div>
                                    </div>
                                </section>
                            ...
                            <!-- categories -->
                            <div v-if="form.discipline_id" class="w-full px-4 py-5 sm:p-6">
                                <label for="categories" class="block mb-4 text-lg font-medium text-gray-700">
                                    Categories d'activité
                                    <span class="text-base italic text-gray-600">(Selectionnez une ou plusieurs
                                        categories)</span>
                                </label>
                                <div class="mt-1">
                                    <ul
                                        ...
                                        <li v-for="categorie in categoriesList" :key="categorie.id" class="py-2">
                                            <div class="flex items-center justify-between">
                                                <input name="categories" id="categories" v-model="form.categories_id"
                                                    type="checkbox" multiple :value="categorie.id" :checked="form.categories_id.includes(
                                                        categorie.id
                                                    )
                                                        "
                                ...
                                <div v-if="form.errors.categories_id" class="mt-2 text-xs text-red-500">
                                    {{ form.errors.categories_id }}
                                </div>
                            </div>

                            <!--buttons -->
                            <div v-if="form.categories_id.length > 0" class="px-4 py-3 text-right md:px-6">
                            ...
                            <section v-if="activites.length > 0"
                                ...
                                <div v-for="(
                                    ...
                                    <div class="flex flex-col justify-start w-full space-y-2">
                                        <div v-if="activite"
                                            class="flex items-center justify-between w-full px-2 text-lg font-semibold text-gray-700">
                                            <p>
                                                {{ activite.disciplineName }}
                                                <span v-if="activite.count > 1" class="text-sm text-gray-600">({{
                                                    activite.count }}
                                                    activités liées)
                                                </span>
                                            ...
                                            openDeleteModal(activite)
                                                ...
                                                <TrashIcon class="w-6 h-6 mr-1 text-gray-600 hover:text-red-600" />
                                            </button>
                                        </div>
                                        <div v-if="activite.categories"
                                            class="grid grid-cols-1 gap-4 place-items-stretch sm:grid-cols-2 lg:grid-cols-4">
                                            <div class="flex flex-col items-center justify-between px-4 py-3 text-lg text-gray-600 transition duration-150 bg-white rounded shadow-lg hover:bg-darkblue hover:text-white hover:ring-2 hover:ring-green-400 hover:ring-offset-2 focus:ring-2 focus:ring-green-400 focus:ring-offset-2 sm:rounded-lg"
                                                v-for="(
                                                    categorie, index
                                                ) in activite.categories" :key="categorie.id">
                                                <Link :href="route(
        ...
        <ModalDeleteCategorie :structure="structure" :categorie="currentCategorie" :show="showDeleteCategorieModal"
            @close="showDeleteCategorieModal = false" />
        <ModalDeleteDiscipline :structure="structure" :activite="currentActivite" :show="showDeleteDisciplineModal"
            @close="showDeleteDisciplineModal = false" @deleteDiscipline="removeDiscipline" />
    </AppLayout>
</template>
