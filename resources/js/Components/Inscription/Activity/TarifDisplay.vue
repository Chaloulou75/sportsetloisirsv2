...
<template>
    <div v-if="structure.tarifs.length === 0 && tarifsList.length === 0">
        <p class="font-semibold text-gray-600 italic">Pas de tarif associé à cette structure</p>
    </div>
    <div v-if="structure.tarifs.length > 0" class="min-h-full w-full rounded-xl shadow-lg mt-6 overflow-x-auto">
    ...
    <div v-if="tarifsList.length > 0" class="min-h-full w-full rounded-xl shadow-lg mt-12 overflow-x-auto">
        <table class="table-fixed md:table-auto border-collapse text-sm font-semibold text-gray-700 w-full">
            <caption class="bg-slate-50 caption-top text-sm font-semibold text-slate-600 py-6">
